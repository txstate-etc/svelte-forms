...
*  `preloadAsDraft` - (optional) Use this when your `preload` data is partial - e.g. a draft pulled from a previous autosave session or a few fields gathered from a third-party system - and the user is still in the middle of filling out the form for the first time. Normally a `preload` marks the entire form dirty so every validation error is visible immediately. With `preloadAsDraft`, the form stays clean and only the fields up through the last preloaded (non-empty) field are dirtied, so the user isn't alarmed by required-field errors for fields they haven't reached yet. This is the same dirtying behavior `autoSave` uses by default; set `preloadAsDraft` when you want that behavior without enabling autosave.
### Events
* `on:saved` - Fires after the user submits the form and the submission is successful.
* `on:autosaved` - Fires after a successful save that was automatically triggered instead of requested by the user. Only happens in `autoSave` mode. It fires once everything the user has entered so far is saved - if they kept typing while a save was in flight, the event holds until the follow-up save that carries those keystrokes lands. That makes it safe to refresh server data in this handler: a snapshot you fetch when it fires will never be missing input that was still being saved, so it can't arrive later and revert the form to stale data.
