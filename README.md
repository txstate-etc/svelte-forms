...
* `number`, `notNull`, `date`, `datetime` - These are all convenience booleans that set `serialize` and `deserialize` to reasonable defaults for your situation. They have no effect if you set `serialize` and `deserialize`. Use up to one of `number`, `date`, or `datetime` (using none of the three implies you're working with `string`). If you're working with number or string, you can set `notNull` and null/undefined will be serialized to `0` and `''` respectively. `notNull` is ignored for `date` and `datetime`, which are always treated as nullable since there is no obvious empty/zero value for an unset date.
